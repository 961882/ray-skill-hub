...
export function buildClaudeCodeSkillDocument(manifest: NonNullable<SkillDescriptor["manifest"]>, sourceContent: string): string {
  return buildFrontmatterSkillDocument({
    name: manifest.name,
    description: resolveSkillDescription(manifest, "claude-code"),
    sourceContent
  });
}
...
export class ClaudeCodeAdapter implements AgentAdapter {
  ...
  async sync(input: SyncInput): Promise<SyncResult> {
    ...
    if (manifest && sourceEntryPath) {
      ...
      const renderedContent = buildClaudeCodeSkillDocument(manifest, sourceContent);
