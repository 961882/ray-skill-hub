...
import { buildFrontmatterSkillDocument, resolveSkillDescription, toDisplayName } from "./frontmatter-skill.js";
import { expandHome } from "../core/paths.js";
import type { AgentDefinition } from "../types/agent.js";
import type { SkillDescriptor, SkillManifest } from "../types/skill.js";

export function resolveCodexDescription(manifest: SkillManifest): string {
  return resolveSkillDescription(manifest, "codex");
}

export function buildCodexSkillDocument(manifest: SkillManifest, sourceContent: string): string {
  return buildFrontmatterSkillDocument({
    name: toDisplayName(manifest.name),
    description: resolveCodexDescription(manifest),
    sourceContent
  });
}
...
export class CodexAdapter implements AgentAdapter {
  ...
  async sync(input: SyncInput): Promise<SyncResult> {
    ...
    if (manifest && sourceEntryPath) {
      ...
      const renderedContent = buildCodexSkillDocument(manifest, sourceContent);
